...
to go
  ...
  if ticks = 1[
    ...
    do_rescale_skill
    ifelse terrain?[
      ask patches[set battle_advantage random-float 1]][ask patches[set battle_advantage 1]
    ]
    do_rank
    if remove_circle?[
      ask patches[
...
to do_rescale_skill
  ...
  ask players[
    set skill ((skill - min_skill) / max_skill)
  ]
end
...
to do_move_and_battle
  ask players[
    ...
    if [pcolor] of patch-here = white[
      ifelse random-float 1 < skill and skillful_movement?[
        let options patches in-radius 1
        move-to max-one-of options [battle_advantage]
        ][rt random-float 360 fd 1.5
      ]
    ]
    if count other players in-radius 1.5 > 0[
      set nr_encounters (nr_encounters + 1)
      let encounter one-of other players in-radius 1.5
      ask encounter [
        set nr_encounters (nr_encounters + 1)
        set positioning [battle_advantage] of patch-here
      ]
      set positioning [battle_advantage] of patch-here
